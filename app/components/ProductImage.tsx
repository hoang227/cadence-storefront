  ...
  return (
    <>
      ...
      <div className="space-y-4">
        ...
        <div
          className="aspect-square relative rounded-lg overflow-hidden bg-brand-cream cursor-zoom-in w-full"
        ...
        <div className="flex md:hidden justify-center space-x-2 mt-4">
          {allImages.map((_, index) => (
            <button
              key={`dot - ${index}`}
              onClick={() => setSelectedIndex(index)}
              className={`rounded-full transition-all duration-300 ${selectedIndex === index ? 'bg-brand-gold w-4' : 'bg-brand-navy/20 hover:bg-brand-navy/40 w-2 h-2'}`}
            ></button>
          ))}
        </div>
      </div>
        ...
        <div className="fixed top-0 left-0 !my-0 inset-0 z-50 bg-black/95 backdrop-blur-sm">
          <div className="absolute inset-0 overflow-hidden">
            ...
            <div className="absolute inset-0 hidden md:flex items-center justify-between px-4">
              ...
              >
                <ChevronRight className="w-8 h-8 text-brand-navy" />
              </button>
            </div>
          </div>
        </div>
